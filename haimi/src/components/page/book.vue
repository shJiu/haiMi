<template>
	<div class="wrap">
		<div v-if="mess.length!=0" class="gouwu">
			<div class="title">
				<i>海蜜严选</i>
			</div>
			<div class="shopp" v-for="(item,index) in mess">
				<div class="ck">
				...
				<dl>
					...
					<dd>
						<p><span>{{item.title}}</span><span></span><span @click="del(index)">删除</span></p>
						<p><span>{{item.color}}</span></p>
						<p><span>￥{{item.price}}</span><span></span><span>x{{item.count}}</span></p>	
					</dd>
				</dl>
			</div>
